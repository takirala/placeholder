...
import (
	...
	"github.com/nutanix-cloud-native/nkp-nutanix-product-catalog/apptests/net"
	"inet.af/netaddr"
)
...
func (d *docker) GetNetwork(ctx context.Context, name string) (bool, *NetworkResource, error) {
	results, err := d.NetworkList(ctx, network.ListOptions{Filters: filters.NewArgs(filters.Arg("name", name))})
	if err != nil {
...
func (d *docker) CreateNetwork(ctx context.Context, name string, internal bool, subnet string) (*NetworkResource, error) {
	...
	resource, err := d.NetworkInspect(ctx, resp.ID, network.InspectOptions{})
	if err != nil {
		return nil, err
	}
